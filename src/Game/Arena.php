...
class Arena
{
    ...
    public function move(Fighter $fighter, string $direction)
    {
        ...
        $destinationX = $xxx + self::DIRECTIONS[$direction][0];
        $destinationY = $yyy + self::DIRECTIONS[$direction][1];

        if (
            $destinationX < 0 || $destinationX >= $this->getSize() ||
            $destinationY < 0 || $destinationY >= $this->getSize()
        ) {
